    ...
    try {
      ...
      while (this.#buffer.length >= 2) {
        ...
        if (length === 126) {
          ...
          if (wide > BigInt(MAX_FRAME_BYTES)) throw new Error('WebSocket frame is too large');
          length = Number(wide);
          offset = 10;
        }
...
export function connectWebSocket(endpoint, extraHeaders = {}, timeout = 15_000) {
  return new Promise((resolve, reject) => {
    const url = new URL(endpoint);
    if (!['ws:', 'wss:'].includes(url.protocol)) {
      reject(new Error(`Expected a ws:// or wss:// endpoint, got ${url.protocol}`));
      return;
    }
    ...
    const timer = setTimeout(() => request.destroy(new Error('WebSocket connection timed out')), timeout);
